...
import { categories, formatPrice, materials } from './shop/materials';
import type { Material, MaterialCategory } from './shop/materials';
...
export default function Shop() {
  ...
  const [category, setCategory] = useState<MaterialCategory | 'Alla material'>('Alla material');
  ...
  useEffect(() => {
    ...
    document.title = 'Fritidsmaterial | Karnor materialbutik';
    window.scrollTo(0, 0);
  ...
  const filtered = materials.filter(material =>
    (category === 'Alla material' || material.category === category) &&
  ...
  const hasFilters = Boolean(query) || category !== 'Alla material' || age !== 'Alla åldrar' || favoritesOnly;

  function resetFilters() { setSearch(''); setCategory('Alla material'); setAge('Alla åldrar'); setFavoritesOnly(false); }

  return <main className="shop-page shop-ui" id="shop-main">
    <div className="shop-preview-banner"><span className="shop-preview-dot" /> En liten förhandstitt på vår materialbutik <span className="shop-preview-divider">·</span><span>Exempelmaterial – köp öppnar senare</span></div>
    <div className="shop-container">
      ...
      <section className="shop-hero" aria-labelledby="shop-title">
        ...
        <div className="shop-hero-art" aria-hidden="true"><div className="shop-hero-orbit" /><span className="shop-hero-spark shop-hero-spark--one">✳</span><span className="shop-hero-spark shop-hero-spark--two">✦</span>
          <div className="shop-hero-book shop-hero-book--back"><MaterialCover material={materials[1]} /></div>
          <div className="shop-hero-book shop-hero-book--front"><MaterialCover material={materials[0]} /></div>
      ...
      <section id="material" className="shop-catalog" aria-labelledby="material-title">
        ...
        <div className="shop-category-list" role="group" aria-label="Filtrera på kategori">{(['Alla material', ...categories] as const).map(item => <button key={item} type="button" className={category === item ? 'is-active' : ''} aria-pressed={category === item} onClick={() => setCategory(item)}>{item}{item === 'Alla material' && <span>{materials.length}</span>}</button>)}</div>
        ...
        {filtered.length > 0 ? <div className="shop-product-grid">{filtered.map(material => {
          ...
          return <article className="shop-product-card" key={material.id}>
            <div className="shop-card-visual"><button type="button" className="shop-cover-button" onClick={() => setSelected(material)} aria-label={`Förhandsvisa ${material.title}`}><MaterialCover material={material} /></button>{material.badge && <span className="shop-product-badge">{material.badge}</span>}<button type="button" className={`shop-icon-button shop-card-favorite ${isFavorite ? 'is-favorite' : ''}`} aria-pressed={isFavorite} aria-label={`${isFavorite ? 'Ta bort' : 'Spara'} ${material.title} ${isFavorite ? 'från' : 'som'} favorit`} onClick={() => toggleFavorite(material.id)}><HeartIcon /></button><button type="button" className="shop-quick-preview" onClick={() => setSelected(material)}><EyeIcon aria-hidden="true" /> Förhandsvisa</button></div>
            <div className="shop-card-copy"><span className="shop-card-category">{material.category}</span><h3><button type="button" onClick={() => setSelected(material)}>{material.title}</button></h3><p className="shop-card-meta">PDF · {material.pages} sidor <span>Åk {material.ages.length === 2 ? 'F–6' : material.ages[0]}</span></p><div className="shop-card-bottom"><strong>{formatPrice(material.price)}</strong><button type="button" className={`shop-add-button ${inCart ? 'is-added' : ''}`} onClick={() => inCart ? openCart() : addToCart(material)} aria-label={`${inCart ? 'Visa varukorgen med' : 'Lägg i varukorg:'} ${material.title}`}>{inCart ? <CheckIcon aria-hidden="true" /> : <ShoppingBagIcon aria-hidden="true" />}<span>{inCart ? 'I varukorgen' : 'Lägg i varukorg'}</span></button></div></div>
          </article>;
        })}</div> : <div className="shop-no-results"><MagnifyingGlassIcon aria-hidden="true" /><h3>{favoritesOnly && favorites.length === 0 ? 'Dina favoriter väntar på att bli hittade' : 'Inga material hittades just nu'}</h3><p>{favoritesOnly && favorites.length === 0 ? 'Tryck på hjärtat vid ett material så samlas det här under ditt besök.' : 'Prova ett annat sökord eller välj en annan kategori.'}</p><button type="button" className="shop-button shop-button--secondary" onClick={resetFilters}>Visa alla material</button></div>}
        <p className="shop-catalog-disclaimer">Produkterna och priserna är exempel. Favoriter och varukorg finns bara kvar under ditt besök och återställs när sidan laddas om.</p>
      </section>
      <section className="shop-how-it-works" aria-labelledby="shop-how-title"><div><span className="shop-eyebrow">FRÅN IDÉ TILL AKTIVITET</span><h2 id="shop-how-title">En liten hjälp.<br />Mycket upptäckarglädje.</h2><p>Så kommer det att fungera när butiken öppnar.</p></div><ol><li><span>01</span><h3>Hitta din inspiration</h3><p>Bläddra bland materialen och kika på ett smakprov.</p></li><li><span>02</span><h3>Välj dina favoriter</h3><p>Samla det som passar er i varukorgen och gå till kassan.</p></li><li><span>03</span><h3>Gör något fint av dagen</h3><p>Ladda ner, skriv ut och låt idéerna ta plats på fritids.</p></li></ol></section>
      ...
      <footer className="shop-footer"><Link to="/" className="shop-footer-brand">Karnor<span>För fritidshemmets alla möjligheter.</span></Link><nav aria-label="Butikens sidfot"><a href="#material">Alla material</a><Link to="/about">Om Karnor</Link><Link to="/planering">Pedagogisk planering</Link></nav><span>Med omtanke, för fritids.</span></footer>
    </div>
    {selected && <ProductDialog key={selected.id} material={selected} onClose={() => setSelected(null)} />}
  </main>;
}
