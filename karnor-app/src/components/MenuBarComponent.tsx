  ...
  return <><a href="#main-content" className="site-skip-link">Hoppa till innehållet</a><Disclosure as="nav" aria-label="Huvudmeny" className="site-header">
    {({ open }) => <><div className="site-header-inner">
      ...
      <div className="site-desktop-nav">{navigation.map(([name, href]) => <NavLink key={href} to={href} className={({ isActive }) => isActive ? 'is-active' : ''}>{name}</NavLink>)}
        <Popover className="site-tools"><PopoverButton>Verktyg & inspiration <ChevronDownIcon aria-hidden="true" /></PopoverButton><PopoverPanel className="site-tools-panel">{({ close }) => <>{tools.map(([name, href]) => <Link key={href} to={href} onClick={() => close()}>{name}</Link>)}<a href="/verksamhet.pdf" target="_blank" rel="noopener noreferrer">Verksamhetsberättelse ↗</a><a href="https://www.skolverket.se/undervisning/fritidshemmet/laroplan-for-fritidshemmet" target="_blank" rel="noopener noreferrer">Läroplanen ↗</a></>}</PopoverPanel></Popover>
      </div>
    ...
    </div><DisclosurePanel className="site-mobile-menu">{[...navigation, ...tools].map(([name, href]) => <DisclosureButton key={href} as={NavLink} to={href}>{name}</DisclosureButton>)}<a href="/verksamhet.pdf" target="_blank" rel="noopener noreferrer">Verksamhetsberättelse ↗</a><a href="https://www.skolverket.se/undervisning/fritidshemmet/laroplan-for-fritidshemmet" target="_blank" rel="noopener noreferrer">Läroplanen ↗</a></DisclosurePanel></>}
  </Disclosure></>;
}
